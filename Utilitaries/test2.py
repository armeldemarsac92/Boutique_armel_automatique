            ...
            for size, quantity in cloth_data[category].items():
                ...
                for brand_id in brand_ids:
                    ...
                    if quantity < expected_quantity:
                        ...
                        status_placeholder.info(f"Lancement de la recherche de {pieces_a_chercher} {category} de la marque {brand_catalog(brand_id)} en {size}...")
                        ...
                        while process.poll() is None:
                            ...
                            with open("../Assets/Data/progress_bar_data.txt", "r") as f:
                                ...
                                progress_placeholder.progress(normalized_progress, f"Avancement du restockage pour {brand_catalog(brand_id)} en {size} : {progress}%.")
                        ...
                        index = stdout_str.find('pieces ont ete collectees')

                        # If the phrase was found
                        if index != -1:
                            # Get the part of the stdout before the phrase
                            before_phrase = stdout_str[:index]

                            # Split this part into words
                            words = before_phrase.split()

                            # The last word should be the number of pieces
                            if words:
                                collected_pieces_count = int(words[-1])
                            else:
                                print('No number found before the phrase')
                        else:
                        ...
                        i += 1
                        normalized_progress_2 = i / (tailles_inferieures*len(brand_ids))
                        progress_placeholder_2.progress(normalized_progress_2,
                        f"Avancement du restockage pour {category} : {i}/{tailles_inferieures*len(brand_ids)} tailles.")
